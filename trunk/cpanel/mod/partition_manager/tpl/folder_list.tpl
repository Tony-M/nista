	...
	<table  border="0" cellspacing="1" cellpadding="0" width="100%" >
		...
		{section name=file_list_num loop=$DOCUMENT.mod.data.catalog_list}
			<tr  onMouseOver="this.style.background='#F4FAFF'" onMouseOut="this.style.background='none'">
				<td class="td_body" align="center">{if $DOCUMENT.mod.data.catalog_list[file_list_num].status == "sys"}<img src="{$DOCUMENT.ACP_IMG_WAY}lock.gif" width="16" height="16" alt="Системный каталог. Закрыт" title="Системный каталог. Закрыт">{else}<input name="selected_path" id="selected_path" value="{$DOCUMENT.mod.data.catalog_list[file_list_num].path}" type="radio">{/if}</td>	
